...
:root{color-scheme:light dark}*{box-sizing:border-box}body{font-family:var(--vscode-font-family);font-size:13px;color:var(--vscode-foreground);margin:0;padding:14px;line-height:1.4}.shell{max-width:760px;margin:0 auto}.header{display:flex;align-items:center;gap:12px;margin-bottom:20px}.title{font-size:18px;font-weight:600;min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis}.nav{display:flex;gap:5px}.nav-button{display:grid;place-items:center;width:32px;height:32px;padding:0;border:0;border-radius:7px;color:var(--vscode-descriptionForeground);background:transparent;cursor:pointer}.nav-button:hover{color:var(--vscode-foreground);background:var(--vscode-toolbar-hoverBackground)}.nav-button.active{color:var(--vscode-button-foreground);background:var(--vscode-button-background)}.nav-button svg{width:17px;height:17px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.view[hidden]{display:none}.page-title{font-size:14px;font-weight:600;margin-bottom:10px}.actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.action,.add,.remove{font:inherit;border:0;border-radius:6px;cursor:pointer}.action{display:flex;align-items:center;gap:8px;min-height:40px;padding:8px 11px;color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground);text-align:left}.action:hover{background:var(--vscode-button-secondaryHoverBackground)}.action svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.section+.section{margin-top:20px}.section-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:7px}.section-title{font-size:13px;font-weight:600}.count{color:var(--vscode-descriptionForeground);font-size:11px}.panel{border:1px solid var(--vscode-panel-border);border-radius:8px;background:var(--vscode-sideBar-background);overflow:hidden}.installed{display:flex;align-items:center;gap:8px;min-height:36px;padding:6px 8px 6px 11px;border-bottom:1px solid var(--vscode-panel-border)}.installed:last-child{border-bottom:0}.package-name{min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis}.remove{width:24px;height:24px;flex:none;color:var(--vscode-descriptionForeground);background:transparent;font-size:16px;line-height:1}.remove:hover{color:var(--vscode-errorForeground);background:var(--vscode-toolbar-hoverBackground)}.empty{padding:14px;text-align:center;color:var(--vscode-descriptionForeground)}.search-wrap{position:relative}.search-icon{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--vscode-descriptionForeground);pointer-events:none}input{width:100%;height:34px;padding:0 34px 0 30px;border:1px solid var(--vscode-input-border,var(--vscode-panel-border));border-radius:7px;color:var(--vscode-input-foreground);background:var(--vscode-input-background);font:inherit;outline:none}input:focus{border-color:var(--vscode-focusBorder)}.spinner{display:none;position:absolute;right:10px;top:9px;width:14px;height:14px;border:2px solid var(--vscode-panel-border);border-top-color:var(--vscode-progressBar-background);border-radius:50%;animation:spin .7s linear infinite}.loading .spinner{display:block}@keyframes spin{to{transform:rotate(360deg)}}.results{margin-top:7px}.result{display:grid;grid-template-columns:1fr auto;gap:6px 10px;padding:10px 11px;border-bottom:1px solid var(--vscode-panel-border)}.result:last-child{border-bottom:0}.result-name{font-weight:600}.description{margin-top:2px;color:var(--vscode-descriptionForeground);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:5px;color:var(--vscode-descriptionForeground);font-size:10px}.chip{font-size:10px}.add{align-self:center;padding:5px 10px;color:var(--vscode-button-foreground);background:var(--vscode-button-background)}.add:hover{background:var(--vscode-button-hoverBackground)}.add:disabled{opacity:.55;cursor:default}.error{padding:10px;color:var(--vscode-errorForeground)}@media(max-width:380px){.header{align-items:flex-start;flex-direction:column}.actions{grid-template-columns:1fr}}
</style></head><body><main class="shell"><header class="header"><div class="title">${escapeHtml(title)}</div>${hasProject ? `<nav class="nav" aria-label="Project manager sections"><button class="nav-button" data-page="actions" title="Actions" aria-label="Actions"><svg viewBox="0 0 24 24"><path d="m8 5 11 7-11 7Z"/></svg></button><button class="nav-button" data-page="dependencies" title="Dependencies" aria-label="Dependencies"><svg viewBox="0 0 24 24"><path d="m12 3 8 4.5v9L12 21l-8-4.5v-9Z"/><path d="m4 7.5 8 4.5 8-4.5M12 12v9"/></svg></button><button class="nav-button" data-action="settings" title="Settings" aria-label="Settings"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1A1.7 1.7 0 0 0 9 4.6 1.7 1.7 0 0 0 10 3V2.8h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.2v4H21a1.7 1.7 0 0 0-1.6 1Z"/></svg></button></nav>` : ""}</header>${hasProject ? `<section class="view" data-view="actions" hidden><div class="page-title">Actions</div><div class="actions"><button class="action" data-action="build"><svg viewBox="0 0 24 24"><path d="m14 5 5 5-9 9H5v-5Z"/><path d="m13 6 5 5"/></svg>Build</button><button class="action" data-action="run"><svg viewBox="0 0 24 24"><path d="m8 5 11 7-11 7Z"/></svg>Run</button><button class="action" data-action="test"><svg viewBox="0 0 24 24"><path d="M9 3h6M10 3v5l-5 9a2 2 0 0 0 2 3h10a2 2 0 0 0 2-3l-5-9V3M8 14h8"/></svg>Test</button><button class="action" data-action="fetch"><svg viewBox="0 0 24 24"><path d="M12 3v12m0 0 5-5m-5 5-5-5M5 21h14"/></svg>Fetch</button></div></section><section class="view" data-view="dependencies" hidden><div class="section"><div class="section-head"><span class="section-title">Dependencies</span><span class="count">${dependenciesList.length}</span></div><div class="panel">${dependenciesList.map(dep => `<div class="installed"><div class="package-name">${escapeHtml(dep.name)}</div><button class="remove" title="Remove ${escapeHtml(dep.name)}" aria-label="Remove ${escapeHtml(dep.name)}" data-remove="${escapeHtml(dep.name)}">×</button></div>`).join("") || `<div class="empty">No dependencies</div>`}</div></div><div class="section"><div class="section-head"><span class="section-title">Add dependency</span></div><div class="search-wrap" id="searchWrap"><span class="search-icon">⌕</span><input id="search" autocomplete="off" spellcheck="false" placeholder="Search packages" aria-label="Search Zig packages"><span class="spinner"></span></div><div class="panel results" id="results" hidden></div></div></section>` : `<div class="panel empty">Open a folder containing build.zig.zon.</div>`}</main><script nonce="${nonce}">
const vscode=acquireVsCodeApi();const input=document.getElementById('search');const results=document.getElementById('results');const wrap=document.getElementById('searchWrap');let timer;function changePage(page){document.querySelectorAll('[data-view]').forEach(view=>view.hidden=view.dataset.view!==page);document.querySelectorAll('[data-page]').forEach(button=>{const active=button.dataset.page===page;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active))});vscode.setState({page})}function el(tag,className,text){const node=document.createElement(tag);if(className)node.className=className;if(text)node.textContent=text;return node}function render(items){wrap?.classList.remove('loading');results.replaceChildren();results.hidden=false;if(!items.length){results.append(el('div','empty','No matching packages'));return}for(const item of items){const row=el('div','result');const copy=el('div');copy.append(el('div','result-name',item.name));copy.append(el('div','description',item.description||'Zig package'));const meta=el('div','meta');for(const value of [item.host,item.minimumZigVersion&&('Zig '+item.minimumZigVersion),item.license,item.stars?('★ '+item.stars):''].filter(Boolean))meta.append(el('span','chip',value));copy.append(meta);const add=el('button','add','Add');add.dataset.packageId=item.id;row.append(copy,add);results.append(row)}}changePage(vscode.getState()?.page||'dependencies');input?.addEventListener('input',()=>{clearTimeout(timer);const query=input.value.trim();if(query.length<2){wrap.classList.remove('loading');results.hidden=true;return}wrap.classList.add('loading');timer=setTimeout(()=>vscode.postMessage({type:'search',query}),280)});document.addEventListener('click',event=>{const target=event.target.closest('button');if(!target)return;if(target.dataset.page){changePage(target.dataset.page);return}if(target.dataset.remove)vscode.postMessage({type:'remove',name:target.dataset.remove});if(target.dataset.packageId){target.disabled=true;target.textContent='Adding…';vscode.postMessage({type:'add',packageId:target.dataset.packageId})}if(target.dataset.action)vscode.postMessage({type:target.dataset.action})});window.addEventListener('message',event=>{if(event.data.type==='results')render(event.data.items);if(event.data.type==='searchError'){wrap?.classList.remove('loading');results.hidden=false;results.replaceChildren(el('div','error',event.data.message))}});
</script></body></html>`;
...
class ProjectDashboard implements vscode.WebviewViewProvider {
  ...
  private async handle(message: { type: string; name?: string; query?: string; packageId?: string }): Promise<void> {
    ...
    if (message.type === "settings") { await vscode.commands.executeCommand("workbench.action.openSettings", "@ext:zig-lang-support.zig-lang-support"); return; }
